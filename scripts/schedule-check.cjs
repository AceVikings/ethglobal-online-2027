...
run(async () => {
  ...
  const plan = { action: 'HIP-423 scheduled HCS marker', topicId: process.env.HCS_TOPIC_ID || '<HCS_TOPIC_ID>', validForSeconds, recurring: false, triggersAgent: false }
  if (!execute) return output({ mode: 'dry-run', ...plan, note: 'This records one delayed ledger marker. It neither invokes HTTP nor makes the caretaker recurring.' })
  const topicId = required(process.env, 'HCS_TOPIC_ID')
  const { TopicMessageSubmitTransaction, Timestamp } = require('@hiero-ledger/sdk')
  ...
  try {
    ...
    output({ mode: 'execute', scheduleId: receipt.scheduleId.toString(), transactionId: response.transactionId.toString(), recurring: false, triggersAgent: false })
