...
if [[ "${1:-}" != "--execute" ]]; then
  echo 'dry-run: VERDICT_SIGNER_KEY=0x... HEDERA_OPERATOR_KEY=0x... forge script script/DeployClearingEscrowV2.s.sol:DeployClearingEscrowV2 --rpc-url https://testnet.hashio.io/api --broadcast'
  exit 0
fi
...
forge script script/DeployClearingEscrowV2.s.sol:DeployClearingEscrowV2 \
  --rpc-url "$RPC_URL" \
