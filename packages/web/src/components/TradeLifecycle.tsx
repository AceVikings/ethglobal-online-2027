  ...
  return (
    <ol className="grid gap-0 border border-hairline bg-surface sm:grid-cols-3 lg:grid-cols-6" aria-label="Confirmed trade lifecycle">
      {stages.map((stage, index) => {
        const event = events.find((candidate) => stage.types.some((type) => type === candidate.type));
        return (
          <li key={stage.label} className="relative min-h-28 border-b border-hairline p-4 last:border-b-0 sm:border-b-0 sm:border-r sm:[&:nth-child(3)]:border-r-0 lg:[&:nth-child(3)]:border-r lg:last:border-r-0">
            {index < stages.length - 1 ? <span className="absolute left-8 top-[31px] hidden h-px w-[calc(100%-2rem)] bg-hairline lg:block" aria-hidden="true" /> : null}
            <span className={`relative z-10 inline-flex h-8 w-8 items-center justify-center rounded-full border ${event ? "border-success bg-success text-white" : "border-hairline bg-surface text-muted-copy"}`}>
